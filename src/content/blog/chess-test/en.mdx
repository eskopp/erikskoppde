...
import diagramm from "./board.png";
...
## A fixed image alongside

When you mean a finished graphic or a screenshot rather than a FEN board, the
`<Figure>` component handles it. It works because this post is saved as `.mdx`:
an optimised image plus an optional caption.

<Figure
  src={diagramm}
  alt="Chessboard with the pawn moving from e2 to e4"
  caption="A static image next to the SVG boards rendered above at build time."
  credit="Graphic: Erik Skopp"
  ai={false}
/>

Every `<Figure>` requires `credit` (rights holder) and `ai` (`true`/`false`).
With `ai={true}` an "AI" badge appears next to the caption.
