...
## Ein festes Bild dazu

Wenn kein FEN-Brett gemeint ist, sondern eine fertige Grafik oder ein
Screenshot, hilft die `<Figure>`-Komponente. Sie funktioniert, weil dieser
Beitrag als `.mdx` gespeichert ist: optimiertes Bild plus optionale
Bildunterschrift.

<Figure
  src={diagramm}
  alt="Schachbrett, der Bauer zieht von e2 nach e4"
  caption="Ein statisches Bild neben den oben zur Build-Zeit gerenderten SVG-Brettern."
  credit="Grafik: Erik Skopp"
  ai={false}
/>

Jede `<Figure>` verlangt `credit` (Rechteinhaber) und `ai` (`true`/`false`).
Mit `ai={true}` erscheint ein „AI"-Abzeichen an der Bildunterschrift.
